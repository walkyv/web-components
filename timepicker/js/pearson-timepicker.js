...
`;
  ...
  class TimePicker extends HTMLElement {
    ...
    onMouseDown(event) {
      const activeEl = this.list.querySelector(`[aria-activedescendant]`);
      ...
      let nextActiveIdx =
        action in actionToIdxMap ? actionToIdxMap[action] : -1;
      ...
      if (nextActiveIdx >= 0) {
        ...
        const isNextVisible = isElementVisible(nextActiveEl, this.list)
        if (!isNextVisible) {
      ...
      if (name === 'open') {
        ...
        if (isOpen) {
          ...
          if (selectedNode !== null) {
            const selectedIcon = selectedNode.querySelector('.pe-icon-wrapper');
